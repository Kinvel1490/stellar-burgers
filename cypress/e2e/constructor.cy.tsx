...
describe('get ingredients test', () => {
  it('ingredient load test', () => {
    ...
    cy.wait('@getIngredients').then(() => {
      cy.get('[data-addIngredient="643d69a5c3f7b9001cfa093c"]')
        .find('.common_button')
        .click();
      cy.get('[data-addIngredient="643d69a5c3f7b9001cfa0941"]')
        .find('.common_button')
        .click();
      cy.get('[data-addIngredient="643d69a5c3f7b9001cfa0943"]')
        .find('.common_button')
        .click();
...
describe('Burger order test', () => {
  it('making order test', () => {
    ...
    cy.wait('@getOrderByNumber').then(() => {
      cy.get('#modals').find('[data-modal="content"]').should('be.visible');
      cy.get('#modals')
        .find('[data-modal="content"]')
      ...
      cy.get('[data-empty="bun-top"]').should('contain.text', 'Выберите булки');
      cy.get('[data-empty="bun-bottom"]').should(
        'contain.text',
        'Выберите булки'
      );
      cy.get('[data-empty="ingredients"]').should(
        'contain.text',
        'Выберите начинку'
      );
    });
